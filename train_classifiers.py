...
from sklearn.model_selection import GridSearchCV, train_test_split, StratifiedGroupKFold, cross_val_score
...
def train_classifiers(X, y, groups):
    """
    Trains multiple classifiers (LightGBM, XGBoost, CatBoost, MLP, SVM-linear, SVM-radial, 
    Logistic Regression, Random Forest, Decision Tree, Naive Bayes) for CDS vs. OHS with 
    Grid Search, optimizing for macro F1 to treat both classes equally. Handles class imbalance.
    Returns the best model and imputer.
    
    Parameters:
    - X: Feature matrix (numpy array)
    - y: Labels (numpy array)
    - groups: Group identifiers (numpy array, e.g., child_id)
    """
    ...
    if sum(y_train == 1) > 0: # Ensure there are positive samples to avoid division by zero
        ...
        ratio = 1.0 # Default if no positive samples (or handle as an error/special case)

    print(f"Class imbalance ratio (count(0)/count(1) for scale_pos_weight): {ratio:.2f}")
    ...
    models = {
        'LightGBM': {
            ...
            'param_grid': {
                ...
                'classifier__max_depth': [3, -1],
                'classifier__n_estimators': [100, 200],
                'classifier__min_child_samples': [20, 50],
                'classifier__scale_pos_weight': [1.0, ratio]  # ~0.33
            }
        },
        'XGBoost': {
    ...
    for model_name, config in models.items():
        ...
        if config['param_grid']:
            grid_search = GridSearchCV(
                ...
                scoring='f1_macro',  # Optimize for balanced performance across CDS and OHS
                cv=5,
                n_jobs=-1,
                verbose=1
            )
...
# Store performance metrics
        ...
        if model_name in ['LightGBM', 'XGBoost', 'CatBoost', 'Random Forest', 'Decision Tree']:
...
if __name__ == "__main__":
    ...
    if args.mode == 'train':
        ...
        if X_features is not None and y_labels is not None and group_ids is not None and len(X_features) > 0:
            ...
            trained_model, trained_imputer = train_classifiers(X_features, y_labels, group_ids)
        else:
